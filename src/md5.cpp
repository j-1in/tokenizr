...
namespace {
const std::uint32_t kShifts[64] = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
...
std::uint32_t round_function(int i, std::uint32_t b, std::uint32_t c, std::uint32_t d,
                             std::uint32_t& g) {
    if (i < 16) {
...
std::uint32_t rotate_left(std::uint32_t x, int n) { return (x << n) | (x >> (32 - n)); }

void transform(const std::uint8_t* block, std::uint32_t& a, std::uint32_t& b, std::uint32_t& c,
               std::uint32_t& d) {
    std::uint32_t m[16];
    for (int i = 0; i < 16; ++i) {
        const int o = 4 * i;
...
void pad_and_finish(const std::uint8_t* data, std::size_t len, std::uint32_t& a, std::uint32_t& b,
...
std::string md5_hex(const std::vector<Byte>& data) {
    ...
    for (std::uint32_t word : words) {
        for (int i = 0; i < 4; ++i) {
            const std::uint8_t byte = static_cast<std::uint8_t>(word >> (8 * i));
