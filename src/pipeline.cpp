...
Results run_pipeline(std::vector<Byte>& input) {
    ...
    LOG(INFO) << "task1 (split + word count + char split): " << elapsed_ms(t0, t1) << " ms";
...
int run_cli(int argc, char** argv) {
    if (argc != 2) {
        LOG(ERROR) << "usage: " << (argc > 0 ? argv[0] : "bpe") << " <input-file>";
        return 2;
    }
    try {
        LOG(INFO) << "OpenMP threads: " << omp_get_max_threads();
        const std::chrono::steady_clock::time_point t_load0 = std::chrono::steady_clock::now();
        std::vector<Byte> input = read_file(argv[1]);
        const std::chrono::steady_clock::time_point t_load1 = std::chrono::steady_clock::now();
        ...
        const std::chrono::steady_clock::time_point t_save1 = std::chrono::steady_clock::now();
        LOG(INFO) << "save output: " << elapsed_ms(t_save0, t_save1) << " ms";
        LOG(INFO) << "wrote output.txt (md5 " << md5_hex(read_file("output.txt")) << ")";
        return 0;
